...
Component({
  ...
  ready() {
    const vehicle = this.properties.vehicle;
    this.setData({
      name:vehicle.name,//姓名
      phone:vehicle.mobile,//电话
      region:vehicle.area.split(' '),// 省市区
      vehicle,
    })
    this.getStoreList()
  ...
  methods: {
    ...
    filter(data){
      ...
      data.some((val, index)=>{
        ...
        if( val.code == vehicle.code ){
          this.setData({
            storeList_index:index,
          })
          return true;
        }
      })
