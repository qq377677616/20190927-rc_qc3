...
Page({
  ...
  isShare() {
    // 判断是否分享进入
    if (!this.data.options.user_id || this.data.options.user_id == wx.getStorageSync('userInfo').user_id) {
  ...
  bindShare() {
    ...
    let share_userId = this.data.share_userId
    ...
    request_05.myVote({
      ...
      router.jump_nav({
        url: `/pages/poster/poster?activity_id=${activity_id}&vote_id=${vote_id}&user_id=${user_id}`,
      })
      request_05.doVote({
        user_id,
        vote_id,
  ...
  initData(options) {
    ...
    if (vote_id != null) {
      ...
      request_05.myVote({
        ...
        console.log("{ user_id, vote_id, hav_rank }", {
          user_id,
          activity_id
        })
